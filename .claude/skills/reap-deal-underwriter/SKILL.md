...
description: Help a Marquette REAP student form a fast, defensible Pursue/Pass opinion on a deal Vito presents in class, ahead of a real 48-hour deadline and majority-vote decision. Vito presents each deal with a SWOT (Strengths/Weaknesses/Opportunities/Threats) and an Unknowns list on the board — this skill builds that same artifact from an OM, rent roll, or raw numbers so the student can prep for the vote fast. This is a screener, not a full underwriting exercise. Use this any time a deal comes in from Vito or a student asks to "screen this deal," "should we pursue this," "help me decide on this deal," "run the numbers," "underwrite this deal," or similar. Every deal gets screened; a full editable underwriting model is optional and only gets built for a deal the class actually votes to pursue, or on explicit request. Always use this skill instead of building an ad hoc spreadsheet or a generic memo when the request involves real estate pro forma inputs, a pursue/pass call, NOI/cash flow projections, IRR/equity multiple/DSCR, or a deal-log entry for REAP.
---
